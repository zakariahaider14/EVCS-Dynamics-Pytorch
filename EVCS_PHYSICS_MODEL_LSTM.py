...
def evaluate_pinn_system_response(env, num_steps=1000):
...
def plot_system_response(time_steps, system_states, save_dir="./figures"):
    ...
    for i in range(NUM_EVCS):
        plt.plot(time_steps, 550*(1+evcs_v_out[:, i]), label=f'EVCS {i+1}', linewidth=2)
    plt.axhline(y= 550*(1 + V_OUT_VARIATION), color='r', linestyle='--', label='Upper Limit')
    ...
    plt.figure()
    for i in range(NUM_EVCS):
        plt.plot(time_steps, 100*(1+evcs_i_out[:, i]), label=f'EVCS {i+1}', linewidth=2)
    plt.xlabel('Time (s)', fontsize=18)
