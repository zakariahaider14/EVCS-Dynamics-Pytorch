...
line_data = torch.tensor(line_data_pu, dtype=torch.float32)

bus_data = np.array([
    [1, 0, 0, 0], [2, 100, 60, 0], [3, 70, 40, 0], [4, 120, 80, 0], [5, 80, 30, 0],
    [6, 60, 20, 0], [7, 145, 100, 0], [8, 160, 100, 0], [9, 60, 20, 0], [10, 160, 120, 0],
...
class ResidualBlock(nn.Module):
    def __init__(self, dim, activation=nn.Tanh()):
        ...
        self.block = nn.Sequential(
            nn.Linear(dim, dim * 2),
            activation,
            nn.LayerNorm(dim * 2),  # Changed from BatchNorm1d to LayerNorm
            nn.Linear(dim * 2, dim),
            activation,
...
def calculate_power_flow_base(v_d, v_q, G, B, mask, eps=1e-8):
...
def physics_loss(model, t, Y_bus_tf, bus_data, evcs_data, attack_actions=None, defend_actions=None):
    ...
    try:
        ...
        if not hasattr(physics_loss, 'last_execution_time'):
            ...
            physics_loss.Kp = torch.ones(NUM_EVCS, dtype=torch.float32) * 8
            physics_loss.Ki = torch.ones(NUM_EVCS, dtype=torch.float32) * 50
        ...
        if not hasattr(physics_loss, 'i_integral'):
            ...
            physics_loss.Kp_i = torch.ones(NUM_EVCS, dtype=torch.float32) * 8
            physics_loss.Ki_i = torch.ones(NUM_EVCS, dtype=torch.float32) * 80
            physics_loss.last_iref = torch.zeros(NUM_EVCS, dtype=torch.float32)
    ...
    # Ensure t has correct shape [batch_size, 1]
            ...
            for i, bus in enumerate(EVCS_BUSES):
                try:
                    ...
                    i_out = safe_op(i_ref)
                    i_dc = torch.clamp(i_dc.clone(), -i_ref, i_ref)
                    ...
                    i_bat_set = safe_op(physics_loss.i_ref[i] + WAC_KP_BATT * wac_error_i_bat + WAC_KI_BATT * wac_integral_i_bat)
                    ...
                    v_dc = safe_op((2 * SQRT_2 / PI) * (v_ref_mag / (overall_modulation_index + 1e-8)))
...
def train_step(model, optimizer, bus_data_batch, Y_bus_tf, bus_data_tf, attack_actions, defend_actions):
    ...
    try:
        ...
        # Calculate all losses with retain_graph=True
        ...
        # Clear the graph after optimization step
        optimizer.zero_grad(set_to_none=True)
        
        return total_loss, {
            'power_flow_loss': power_flow_loss.detach(),
            'evcs_loss': evcs_loss.detach(),
            'wac_loss': wac_loss.detach(),
            'v_reg_loss': v_reg_loss.detach()
        }
    except Exception as e:
...
def sssssss(env, num_steps=1000):
    """Evaluate the PINN-based system response without attacks."""
    try:
        state, _ = env.reset()
        time_steps = []
        system_states = {
        ...
        for step in range(num_steps):
            current_time = step * env.TIME_STEP/env.TIME_STEP
            
            # Create neutral actions (no attack)
            action = {
                'dqn': np.zeros(env.NUM_EVCS),  # Neutral DQN action
                'attacker': np.zeros(env.NUM_EVCS * 2),  # No attack
                'defender': np.ones(env.NUM_EVCS * 2) * 0.5  # Default control parameters
            }
            
            # Take step in environment
            next_state, _, done, _, info = env.step(action)
            
            # Extract and store system states
            num_buses = env.num_buses
            time_steps.append(current_time)
            
            # Store voltage and current information
            system_states['voltages'].append(next_state[:num_buses])
            system_states['currents'].append(next_state[num_buses:2*num_buses])
            
            # Store EVCS-specific states if available
            if 'evcs_states' in info:
                system_states['evcs_states'].append(info['evcs_states'])
            
            # Store power flow information if available
            if 'power_flow' in info:
                system_states['power'].append(info['power_flow'])
...
def evaluate_pinn_system_responffffffse(env, num_steps=1500):
    ...
    """Evaluate the PINN-based system response under normal conditions (no attacks)."""
    try:
        state, _ = env.reset()
        time_steps = []
        system_states = {
        ...
        # Convert Y_bus to complex tensor if it isn't already
        if not env.y_bus_tf.is_complex():
        ...
        for step in range(num_steps):
            ...
            # Calculate power flow if needed
            v_d = voltage_magnitude * torch.cos(voltage_angle)
            v_q = voltage_magnitude * torch.sin(voltage_angle)

            # Store EVCS output values
            v_out = predictions[:, v_out_indices]  # Get v_out from your predictions
            i_out = predictions[:, i_out_indices]  # Get i_out from your predictions
            system_states['evcs_v_out'].append(v_out.detach().numpy()[0])
            system_states['evcs_i_out'].append(i_out.detach().numpy()[0])
                       
            # Use the complex Y_bus for power flow calculations
            power_flow = calculate_power_flow_base(
                v_d, 
                v_q, 
                y_bus_complex.real,  # Now safe to access real part
                y_bus_complex.imag,  # Now safe to access imaginary part
                torch.ones_like(v_d)
            )
...
def plot_system_respossssssnse(time_steps, system_states, save_dir="./figures"):
    """Plot the PINN system response with EVCS output voltage and current."""
    ...
    evcs_v_out = np.array(system_states['evcs_v_out'])  # EVCS output voltage
    evcs_i_out = np.array(system_states['evcs_i_out'])  # EVCS output current
    
    # Plot all bus voltage profiles
    plt.figure(figsize=(12, 6))
    for bus in range(voltages.shape[1]):
        plt.plot(time_steps, voltages[:, bus], label=f'Bus {bus+1}')
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/voltage_profiles_{timestamp}.pdf", bbox_inches='tight')
    plt.close()
    
    # Plot all bus current profiles
    plt.figure(figsize=(12, 6))
    for bus in range(currents.shape[1]):
        plt.plot(time_steps, currents[:, bus], label=f'Bus {bus+1}')
    plt.xlabel('Time (s)', fontsize=18)
    plt.ylabel('Current (p.u.)', fontsize=18)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    # plt.title('Bus Current Profiles')
    plt.legend(bbox_to_anchor=(1.05, 1), fontsize=18)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/current_profiles_{timestamp}.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_output_voltage_profiles_{timestamp}.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_output_current_profiles_{timestamp}.pdf", bbox_inches='tight')
    plt.close()
    
    # Save data to CSV
    data = {
        'Time': time_steps,
        **{f'Voltage_Bus_{i+1}': voltages[:, i] for i in range(voltages.shape[1])},
        **{f'Current_Bus_{i+1}': currents[:, i] for i in range(currents.shape[1])},
        **{f'EVCS_{i+1}_Vout': evcs_v_out[:, i] for i in range(NUM_EVCS)},
        **{f'EVCS_{i+1}_Iout': evcs_i_out[:, i] for i in range(NUM_EVCS)}
    }
    pd.DataFrame(data).to_csv(f"{save_dir}/system_response_{timestamp}.csv", index=False)
...
def plot_system_response(time_steps, system_states, save_dir="./figures"):
    ...
    plt.xlabel('Time (s)', fontsize=18)
    plt.ylabel('Active Power Change (p.u.)', fontsize=18)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    #plt.title('EVCS Active Power Demand', fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_active_power_demand_{timestamp}.pdf")
    plt.close()
    ...
    # plt.axvline(x=load_change_time, color='g', linestyle='--', label='Load Change')
    plt.xlabel('Time (s)', fontsize=18)
    plt.ylabel('Reactive Power Change (p.u.)', fontsize=18)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    # plt.title('EVCS Reactive Power Demand', fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_reactive_power_demand_{timestamp}.pdf")
    plt.close()
    ...
    # plt.axvline(x=load_change_time, color='g', linestyle='--', label='Load Change')
    plt.xlabel('Time (s)', fontsize=18)
    plt.ylabel('Apparent Power Change (p.u.)', fontsize=18)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    # plt.title('EVCS Apparent Power Demand', fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    # plt.title('EVCS Output Voltage Profiles', fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_output_voltage_profiles_{timestamp}.pdf")
    plt.close()
    ...
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    # plt.title('EVCS Output Current Profiles', fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.tight_layout()
