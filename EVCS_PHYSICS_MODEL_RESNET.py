...
def evaluate_pinn_system_response(env, num_steps=1500):
    """Evaluate the PINN-based system response with a load step change."""
    ...
    try:
        ...
        system_states = {
            ...
            'evcs_i_out': [],
            'load_demand': [],
            'active_power': np.zeros((num_steps, NUM_EVCS)),  # Pre-allocate arrays
            'reactive_power': np.zeros((num_steps, NUM_EVCS))  # Pre-allocate arrays
        }
        
        # Calculate load step change time
        load_change_step = int(0.25 * num_steps)
        
        # Initial and final load demands (in p.u.)
        initial_load = 1.0
        final_load = 2.0  # Double the load demand
        ...
        for step in range(num_steps):
            ...
            current_load = final_load if step >= load_change_step else initial_load
            ...
            # Get EVCS output values and apply load change
            v_out = predictions[:, v_out_indices]
            i_out = predictions[:, i_out_indices]
            
            # Apply load change to EVCS buses
            if step >= load_change_step:
                # i_out = i_out * current_load
            ...
            for bus_idx in range(NUM_EVCS):
                ...
                # Assume power factor of 0.95 (typical for EVCS)
                power_factor = 0.95
                
                # Calculate active and reactive power
                P = (S * power_factor).item()  # Convert to scalar
                Q = (S * torch.sin(torch.acos(torch.tensor(power_factor)))).item()  # Convert to scalar
...
def plot_system_response(time_steps, system_states, save_dir="./figures"):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_active_power_demand_{timestamp}.png", bbox_inches='tight')
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_reactive_power_demand_{timestamp}.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_apparent_power_{timestamp}.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    # plt.title('EVCS Power Factor', fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/evcs_power_factor_{timestamp}.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    # plt.title('EVCS Load Demand Profile', fontsize=18)
    plt.legend(fontsize=18)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/load_demand_profile_{timestamp}.pdf", bbox_inches='tight')
    plt.close()
